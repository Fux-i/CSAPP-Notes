...
0000000000400da0 <main>:
  ...
  400e53:	48 89 c7             	mov    %rax,%rdi  # 将 read_line 的返回值（输入的字符串）放入 rdi 作为第一个参数传递给 phase_2 函数
...
0000000000400efc <phase_2>:
  ...
  400f02:	48 89 e6             	mov    %rsp,%rsi  // 栈顶作为第二个参数（rdi是读入的字符串，在main中已经放入rdi）
  400f05:	e8 52 05 00 00       	call   40145c <read_six_numbers>
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)
  400f0e:	74 20                	je     400f30 <phase_2+0x34>  // 第一个数必须为1
  400f10:	e8 25 05 00 00       	call   40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax  // 前一个数
  400f1a:	01 c0                	add    %eax,%eax        // *2
  400f1c:	39 03                	cmp    %eax,(%rbx)      // == 后一个数
  400f1e:	74 05                	je     400f25 <phase_2+0x29>
  400f20:	e8 15 05 00 00       	call   40143a <explode_bomb>
  400f25:	48 83 c3 04          	add    $0x4,%rbx        // 判断下一个数
  400f29:	48 39 eb             	cmp    %rbp,%rbx        // 是否到达结束条件
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx   // rbx指向第二个数
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp  // rbp表示结束条件
...
0000000000400f43 <phase_3>:
  ...
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx // 第二个数，第四个参数
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx // 第一个数，第三个参数
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax
  400f5b:	e8 90 fc ff ff       	call   400bf0 <__isoc99_sscanf@plt>
  400f60:	83 f8 01             	cmp    $0x1,%eax  // 输入参数 > 1
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	call   40143a <explode_bomb>
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>  // arg1 <= 7
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax
  400f75:	ff 24 c5 70 24 40 00 	jmp    *0x402470(,%rax,8)     // 通过switch跳转表间接跳转
...
0000000000400fce <func4>: // 二分查找递归
  ...
  400ff7:	39 f9                	cmp    %edi,%ecx  // until edi >= ecx
...
000000000040100c <phase_4>:
  ...
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi // format string
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax
  401024:	e8 c7 fb ff ff       	call   400bf0 <__isoc99_sscanf@plt>
  401029:	83 f8 02             	cmp    $0x2,%eax  // must read 2 nums
  40102c:	75 07                	jne    401035 <phase_4+0x29>
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) // 14 should <= num1
  ...
  40104f:	75 07                	jne    401058 <phase_4+0x4c>  // eax should == 0
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)   // num2 should == 0
...
0000000000401062 <phase_5>:
  ...
  401067:	48 89 fb             	mov    %rdi,%rbx  // rbx 指向输入的字符串
  ...
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70>  // ax = 0; ⬇️
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx // 取输入字符串的每个字符
  40108f:	88 0c 24             	mov    %cl,(%rsp) // 取低4位
  401092:	48 8b 14 24          	mov    (%rsp),%rdx
  401096:	83 e2 0f             	and    $0xf,%edx
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx  // 映射表
  ...
  4010d9:	48 8b 44 24 18       	mov    0x18(%rsp),%rax  // 检查金丝雀值（是否有栈溢出）
...
00000000004010f4 <phase_6>:
  ...
  401103:	48 89 e6             	mov    %rsp,%rsi      // 用栈接收输入的六个数
  401106:	e8 51 03 00 00       	call   40145c <read_six_numbers>
  40110b:	49 89 e6             	mov    %rsp,%r14      // r14 = sp
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d     // r12d = 0
  401114:	4c 89 ed             	mov    %r13,%rbp      // bp = sp
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax // ax = sp[0]
  40111b:	83 e8 01             	sub    $0x1,%eax      // ax = sp[0] - 1
  40111e:	83 f8 05             	cmp    $0x5,%eax      // 5 <= ax
  ...
  401153:	48 8d 74 24 18       	lea    0x18(%rsp),%rsi  // nums.end()
  401158:	4c 89 f0             	mov    %r14,%rax        // ax = sp
  40115b:	b9 07 00 00 00       	mov    $0x7,%ecx        // cx = 7
  401160:	89 ca                	mov    %ecx,%edx        // dx = 7
  401162:	2b 10                	sub    (%rax),%edx      // dx -= sp[i]
  401164:	89 10                	mov    %edx,(%rax)      // sp[i] = 7 - sp[i]
  401166:	48 83 c0 04          	add    $0x4,%rax        // ax = sp[++i]
  40116a:	48 39 f0             	cmp    %rsi,%rax        // loop
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>
  40116f:	be 00 00 00 00       	mov    $0x0,%esi        // si = 0
  ...
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx // cx = sp[0]
...
000000000040145c <read_six_numbers>:
  ...
  401460:	48 89 f2             	mov    %rsi,%rdx      // 将第一个数的地址（栈帧中的局部变量）存入rdx寄存器，作为sscanf的第三个参数
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx // 第二个数
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax// 第六个数
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp) // 第八个参数，通过栈传递
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax// 第五个数
  401474:	48 89 04 24          	mov    %rax,(%rsp)    // 第七个参数，通过栈传递
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9  // 第四个数
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8  // 第三个数
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi // 格式字符串作为第二个参数
  401485:	b8 00 00 00 00       	mov    $0x0,%eax      // 将eax置0
  40148a:	e8 61 f7 ff ff       	call   400bf0 <__isoc99_sscanf@plt> // 第一个参数在rdi中，在main中设置
  40148f:	83 f8 05             	cmp    $0x5,%eax
  401492:	7f 05                	jg     401499 <read_six_numbers+0x3d> // 若成功读取了5个数以上，跳过爆炸函数
